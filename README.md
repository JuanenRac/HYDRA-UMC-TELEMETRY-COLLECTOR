...
* **Why `sink.InvalidDataError` doesn't change the retry policy, only makes it diagnosable.** `collector.go`'s all-or-nothing requeue-and-retry (see above) stays exactly as it was - a permanently-invalid sample still gets retried like anything else, which is itself a known, documented limitation. What's new is real visibility: `GET /stats`'s `invalidDataErrors` vs. `transportErrors` lets an operator tell "DATALAKE is rejecting our data" apart from "the network to DATALAKE is down" without guessing from logs.
