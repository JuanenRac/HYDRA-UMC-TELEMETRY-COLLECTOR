...
func TestDatalakeSink_InvalidDataErrorNamesTheRealBatchIndex(t *testing.T) {
	var mu sync.Mutex
	callCount := 0
	...
	batch := []telemetry.Sample{
		{SourceID: "a", Kind: "k", Timestamp: 1, Fields: map[string]float64{"v": 1}},
		{SourceID: "b", Kind: "k", Timestamp: 2, Fields: map[string]float64{"v": 2}},
		{SourceID: "poison", Kind: "k", Timestamp: 3, Fields: map[string]float64{"v": 3}},
		{SourceID: "d", Kind: "k", Timestamp: 4, Fields: map[string]float64{"v": 4}},
	}

	invalid, ok := AsInvalidData(sink.Write(batch))
	if !ok {
		t.Fatal("expected a real *InvalidDataError for the 3rd sample's 400")
	}
	if invalid.Index != 2 {
		t.Fatalf("Index = %d, want 2 (the 3rd sample, 0-based) - a wrong Index would make collector.go quarantine the wrong sample entirely", invalid.Index)
	}
	if invalid.Sample.SourceID != "poison" {
