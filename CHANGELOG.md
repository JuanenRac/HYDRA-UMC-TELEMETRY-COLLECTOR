...
All notable work on **HYDRA-UMC-TELEMETRY-COLLECTOR** is summarized here, newest first.
...
- **The bug.** `Collector.FlushOnce` requeued the WHOLE drained batch on
  ...
  invalid (a real HTTP 400 via `sink.InvalidDataError`). Requeuing put
  that same poisoned sample right back at the front of the buffer, where
  the very next flush attempted it again, failed the identical way, and
  requeued it again - forever. Every real, valid sample behind it in the
  queue was blocked indefinitely by one sample that was never going to
  succeed no matter how many times it was retried.
- **The fix.** `sink.InvalidDataError` now carries the exact `Index` of
  the rejected sample within the batch `DatalakeSink.Write` was given
  (it already sends one HTTP request per sample and stops at the first
  failure, so it always knows exactly which one failed). `FlushOnce` uses
  this to quarantine - permanently drop, never requeue - exactly that one
  sample, while the rest of the batch is still requeued and retried
  normally. A transport-level failure (or a sink that can't identify the
  offending sample) still requeues the whole batch, unchanged.
- **Real reporting, not a silent discard.** A new `Stats().Quarantined`
  counter (exposed as `quarantined` in `GET /stats`) counts every sample
  discarded this way - distinct from `invalidDataErrors` (flush attempts,
  not samples) and from `dropped` (a full-buffer capacity limit, not a
  permanent rejection).
- 4 new tests: the exact `[valid, POISONED, valid]` reproduction (only
  the poisoned sample is dropped, the survivors flush successfully on
  the very next attempt), the single-sample-batch edge case, a direct
  `quarantineSample` unit test (including the out-of-range-index
  fallback), and a `DatalakeSink` test confirming `Index` names the real
  batch position, not just the HTTP call number.
