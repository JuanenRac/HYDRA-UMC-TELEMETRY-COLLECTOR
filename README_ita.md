...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/Linguaggio-Go-orange.svg" alt="Go">
  <img src="https://img.shields.io/badge/Protocollo-CAN%20%2F%20WebSocket-yellow.svg" alt="Protocol">
</p>

**Controllo di onestà - cosa funziona davvero oggi:** la vera pipeline di ingestione - `telemetry` che analizza frame CAN e JSON WebSocket in un `Sample` normalizzato, l'anello a capacità limitata con segnalazione di contropressione di `buffer`, l'orchestrazione ingestione+flush di `collector` (una scrittura fallita verso il sink rimette in coda l'intero batch invece di perderlo), il tracciamento della sequenza per produttore resistente alla riconnessione di `dedup`, e il vero `DatalakeSink` di `sink` (testato contro un vero `net/http/httptest.Server` che implementa il `POST /ingest` proprio di DATALAKE) o il suo fallback `ConsoleSink` - sono tutti reali e testati (48 test, `go test ./...`). Due vere lacune da conoscere prima di integrare: questo collector non apre da sé una connessione reale a un bus FDCAN fisico né accetta connessioni WebSocket live da un robot - l'ingestione avviene tramite la sua semplice API HTTP (`POST /ingest/can`, `POST /ingest/ws`), quindi qualcos'altro deve ancora leggere il vero bus/socket e inoltrare qui i frame. E il formato di frame CAN proprio di `telemetry/can.go` (byte del codice segnale + valore float32) è una convenzione v0 propria e autodichiarata di questo progetto, non ancora le vere tabelle di ID CAN documentate dall'ecosistema nel firmware proprio di HYDRA-UMC/URTC - deliberatamente non indovinate. L'output è solo JSON reale; il "JSON/Protobuf" dello stesso README e l'ingestione gRPC restano pianificati, non implementati in nessuna parte di questo codice. Vedi `CHANGELOG.md` per ciò che è stato consegnato esattamente finora.

---

## 1. 🛠️ PANORAMICA TECNICA

**HYDRA-UMC-TELEMETRY-COLLECTOR** è il gateway ad alta velocità che cattura tutte le comunicazioni grezze all'interno dell'ecosistema. Ascolta i bus FDCAN e i flussi WebSocket, incanalando i dati nel Datalake.
...
* 🚀 **Ingestione multi-protocollo:** Gestisce la telemetria CAN e WebSocket oggi, tramite una semplice API di ingestione HTTP. *(l'ingestione gRPC è pianificata)*
* ⚡ **Alto throughput:** Ottimizzato per migliaia di messaggi al millisecondo con un sovraccarico minimo della CPU.
* 🧬 **Normalizzazione dei dati:** Traduce i pacchetti binari grezzi in un formato JSON standardizzato. *(l'output Protobuf è pianificato, non implementato)*
...
* **Perché un fallimento di scrittura nel sink rimette in coda il lotto invece di scartarlo - tranne l'unico campione che non avrà mai successo.** `src/collector` è ciò che sostiene davvero la promessa "Consegna con Buffer: zero perdita di dati": `FlushOnce` rimuove un lotto dal buffer solo quando `Sink.Write` lo conferma, e lo rimette proprio in testa in caso di fallimento di trasporto - un'interruzione reale riprova gli stessi campioni, non li perde. L'unica eccezione è un campione che il sink stesso ha rifiutato in modo PERMANENTE come non valido (vedi `sink.InvalidDataError` più sotto) - rimetterlo in coda incondizionatamente lo riportava in testa per sempre, bloccando ogni campione valido dietro di esso; ora viene invece messo in quarantena (scartato, conteggiato nel `quarantined` di `GET /stats`), mentre il resto del lotto continua a essere rimesso in coda normalmente. Il buffer (`src/buffer`) resta comunque limitato - un'interruzione che dura più della sua capacità SCARTA davvero l'eccesso più vecchio, un limite reale e onesto invece di promettere memoria infinita.
...
* **Perché `sink.InvalidDataError` porta l'`Index` del campione rifiutato all'interno del lotto.** Trovato durante l'audit del codice (P1): `DatalakeSink.Write` invia un campione per richiesta HTTP e si ferma al primo fallimento, quindi quando DATALAKE restituisce un vero HTTP 400 per un campione specifico, `Write` marca la posizione di quel campione nel lotto sull'errore stesso prima di restituirlo. `FlushOnce` usa questo per mettere in quarantena (scartare permanentemente, mai rimettere in coda) esattamente quel campione - riprovare gli stessi byte rifiutati non può mai avere successo - continuando comunque a rimettere in coda normalmente il resto del lotto, invece che l'intero lotto venga ritentato (e ribloccato) per sempre dietro un campione avvelenato. `quarantined` in `GET /stats` conta ogni campione scartato in questo modo, separatamente da `invalidDataErrors` (tentativi di flush, non campioni) e da `dropped` (un limite di capacità del buffer pieno, non un rifiuto permanente) - quindi niente di tutto ciò è uno scarto silenzioso, e `invalidDataErrors` contro `transportErrors` permette comunque a un operatore di distinguere "DATALAKE sta rifiutando i nostri dati" da "la rete verso DATALAKE è inattiva" senza dover indovinare dai log.
